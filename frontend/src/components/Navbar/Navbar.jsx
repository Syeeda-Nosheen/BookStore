    ...
    <nav className="z-50 relative flex bg-zinc-800 text-white px-8 py-2 item-center justify-between">
      ...
      {links.map((items, i) => (
            ...
            {isLoggedIn === false && (
                <>
                <Link to ="/LogIn" 
            className={`${MobileNav}  px-8 py-2 mb-8 border text-3xl mb-8 font-semibold border-blue-500 rounded text-white hover:bg-white hover:text-zinc-800 transition-all duration-300`}>
                LogIn</Link>
            <Link to ="/SignUp" 
            className={`${MobileNav} px-8 py-2 mb-8 bg-blue-500 text-3xl mb-8 font-semibold rounded hover:bg-white hover:text-zinc-800 transition-all duration-300`}>
                SignUp</Link>
                </>
            )}
           </div>
